...
// >>> CUSTOM: HiClaw — detect LLM API errors that need model switch <<<
  ...
  const handleSaveModel = async () => {
    ...
    try {
      ...
      const { data } = await openHands.post(
        `/api/v1/conversations/${conversationId || "unknown"}/switch-model`,
        payload,
      );
    ...
    >
      <div className="flex gap-2 items-start">
        <div className="min-w-0 flex-1">
          <div
            className={cn(
              "whitespace-pre-wrap wrap-break-words text-sm",
          ...
          <div className="flex items-center gap-2 mt-1.5">
            ...
            {showLLMSwitch && !showModelForm && (
              <button
                type="button"
                onClick={handleOpenModelForm}
                className="text-xs px-2 py-1 bg-blue-600 hover:bg-blue-700 rounded font-medium transition"
              >
                更换模型
              </button>
            )}
      ...
      {/* >>> CUSTOM: HiClaw — inline model config form <<< */}
      {showModelForm && (
        <div className="border-t border-red-800/50 pt-2 space-y-2">
          <div className="text-xs text-gray-300 font-medium">快速更换模型配置：</div>
          <div className="grid grid-cols-1 gap-2">
            <input type="text" value={model} onChange={(e) => setModel(e.target.value)}
              placeholder="模型名称 (e.g. openai/qwen-max)"
              className="px-2 py-1.5 bg-[#1a1d24] border border-[#444] rounded text-xs text-white focus:outline-none focus:border-blue-500" />
            <input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)}
              placeholder="API Key (留空则不更改)"
              className="px-2 py-1.5 bg-[#1a1d24] border border-[#444] rounded text-xs text-white focus:outline-none focus:border-blue-500" />
            <input type="text" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)}
              placeholder="Base URL (留空则不更改)"
              className="px-2 py-1.5 bg-[#1a1d24] border border-[#444] rounded text-xs text-white focus:outline-none focus:border-blue-500" />
          </div>
            ...
            {saveMsg && !saveMsg.includes("失败") ? (
              ...
              <>
                <button type="button" onClick={handleSaveModel} disabled={saving}
                  className="text-xs px-3 py-1 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 rounded font-medium transition">
                  {saving ? "切换中..." : "切换模型"}
                </button>
                <button type="button" onClick={() => setShowModelForm(false)}
                  className="text-xs px-3 py-1 bg-[#333] hover:bg-[#444] rounded transition">
                  取消
                </button>
          ...
          <div className="text-[10px] text-gray-500 mt-1">
            切换后容器重启（文件保留），刷新页面发送消息即可以新模型在当前会话继续。
          </div>
        </div>
      )}
      {/* >>> END CUSTOM <<< */}
